...
class WooFi(Base):
    async def swap_eth_to_usdc(self, amount: TokenAmount, slippage: float = 1) -> str:
        ...
        min_to_amount = TokenAmount(
            ...
            decimals=await self.client.transactions.get_decimals(contract_address=to_token.address)
        )
    ...
    async def swap_usdc_to_arb(self, amount: Optional[TokenAmount] = None, slippage: float = 1) -> str:
        ...
        tx = await self.client.transactions.sign_and_send(tx_prarams=tx_params)
        receipt = await tx.wait_for_receipt(client=self.client, timeout=200)
        if receipt:
    ...
    async def swap_arb_to_wbtc(self, amount: Optional[TokenAmount] = None, slippage: float = 1) -> str:
        ...
        await self.improve_interface(token_address=from_token.address, spender=contract.address, amount=amount)
        await asyncio.sleep(5)
