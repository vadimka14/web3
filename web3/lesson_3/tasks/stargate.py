...
class Stargate(Base):
    ...
    async def send_usdc(
        ...
        if dest_fee:
            dst_native_token_price = await self.get_token_price(token_symbol=to_network.coin_symbol)
            dst_native_amount_dollar = float(dest_fee.Ether) * dst_native_token_price
    ...
    @staticmethod
    async def get_network_with_usdc_balance(
            address: ChecksumAddress,
    ) -> Network | None:
        supported_networks = [Networks.BSC, Networks.Optimism, Networks.Avalanche, Networks.Arbitrum]
