...
async def is_there_official_bridge_txs(address: str) -> bool:
    ...
    txs = await Client(network=Networks.Ethereum).transactions.find_txs(
        address=address,
        contract='0x32400084c286cf3e17e7b677ea9583e60a000324',  # address of official bridge
...
async def okx_withdraw(wallet_addresses: list[str]):
    ...
    for num, address in enumerate(wallet_addresses, start=1):
        ...
        if config.use_oficcial_bridge and not official_bridge_is_done:
            # withdraw to eth
            chain = Chains.ERC20
        ...
        elif official_bridge_is_done or not config.use_oficcial_bridge:
            # withdraw to zksync
            chain = Chains.zkSyncEra
        ...
        if not chain or not res:
            logger.error(f'something went wrong: official_bridge_is_done: {official_bridge_is_done} | '
                         f'use_official_bridge:{config.use_oficcial_bridge} | '
                         f'res: {res} | '
                         f'address: {address}')
            continue
